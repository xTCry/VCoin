...
function safeToken(t, n) {
	let m = t.length, n = n || m>6? m/3: 5, s = m - n;
...
function getCurCompletions() {
	let temp = _stateCmd == StateCmd.NONE? "coins, stop, run, buy, tran, price, color, hideupd, autoBuy, autoBuyItem, smartBuy, tspam, beep, debug, datecolorbg":
...
function initIZCap(uid) {
	let defVal = { };
	IZCap = new _izCap("./data/config_"+uid, defVal, false, false);
}
async function configSet(name, value, save) {
	try {
		return await IZCap.set(name, value, save);
	} catch(e) {
