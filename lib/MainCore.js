(async _=> {
	...
	vConinWS.onUserLoaded((place, score, items, top, firstTime, tick)=> {
		...
		con("В топе: " + formateScore(place) + "\tCoins: "+ formateScore(score, true), "yellow");
	...
	rl.on('line', async (line) => {
		...
		switch(line.trim().toLowerCase()) {
			...
			case 'info':
				...
				ccon(" - Coins: "+colors.bold(formateScore(miner.score, true)), "green");
				ccon(" - @ID: "+USER_ID, "green");
				if(g_id) {
					temp = await vConinWS.loadGroup(g_id)
					ccon(" - Group: ["+colors.bold(temp.name)+"] @public"+g_id+"", "green");
				}
				break;
	...
	async function createNewConfig(exit, authType) {
		...
		if(authType == "token") {
			ccon("Если нету Android токена, то получить его можно тут -> " + colors.underline("vk.cc/9gjvSG"));
			ccon("↓↓↓ Введите токен ↓↓↓");
			rl.hideMode = true;
			let _token = await rl.questionAsync("");
			rl.hideMode = false;
			...
			ccon("Желательно указать Android токен. Получить его можно тут -> " + colors.underline("vk.cc/9gjvSG"));
	...
	async function trySelectConfig(list) {
		...
		let data = await rl.questionAsync("Введите номер пользователя [или 0 для создания нового]: ");
			...
			(async function inVKProc(token, owner_id) {
				...
				try {
					...
					try {
						...
						if(e.message && e.message.indexOf("Unknown method passed") !== -1) {
							ccon("Получить рабочий Android токен можно тут -> " + colors.underline("vk.cc/9gjvSG"), "yellow");
